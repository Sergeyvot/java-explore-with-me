...
public class ClientControllerTest {
    ...
    void testAddHit() throws Exception {
        ...
        Mockito.verify(statClient, Mockito.times(1))
                .addPost(any());

        Mockito.verifyNoMoreInteractions(statClient);
    }

    @Test
    void testGetStats() throws Exception {
        when(statClient.getStats(any(), any(), anyList(), anyBoolean()))
                .thenReturn(list);

        mvc.perform(get("/stats?start=2022-11-20T12:00:00&end=2023-11-20T12:00:00&uris=[/events]")
                        .characterEncoding(StandardCharsets.UTF_8)
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
